...
impl<'a> Iterator for WriteBatchIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let key_len =
            u32::from_be_bytes(self.wb.entries[self.pos..self.pos + 4].try_into().unwrap())
                as usize;
        self.pos += 4;
...
mod tests {
    ...
    fn write_and_read() {
        ...
        for i in 0..batch_size {
            wb.append(&(i as i32).to_be_bytes(), &(i as i32).to_be_bytes());
        }
        ...
        for (i, (key, value)) in wb.iter().enumerate() {
            ...
            assert_eq!(i as i32, i32::from_be_bytes(value.try_into().unwrap()));
            items_read += 1;
        }
